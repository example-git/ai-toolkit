...
class StableDiffusion:
    ...
    def load_model(self):
        ...
        #     base_model_path = '/workspace/huggingface/hub/models--black-forest-labs--FLUX.1-dev/snapshots/0ef5fff789c832c5c7f4e127f94c8b54bbcced44'
        #     # Initialize Flux model
        #     flux_params = FluxParams()
        #     transformer = Flux(flux_params)
        
        #     # Load pre-trained weights
        #     # if self.model_config.unet_path is not None:
        #     #     weights_file = os.path.join(self.model_config.unet_path, 'pytorch_model.bin')
        #     # elif os.path.exists(os.path.join(model_path, 'transformer', 'pytorch_model.bin')):
        #     #     weights_file = os.path.join(model_path, 'transformer', 'pytorch_model.bin')
        #     # else:
        #     #     weights_file = hf_hub_download(model_path, filename='pytorch_model.bin', subfolder='transformer')
        
        #     # state_dict = torch.load(weights_file, map_location='cpu')
        #     # transformer.load_state_dict(state_dict)
        #     transformer.to(self.device_torch, dtype=self.torch_dtype)
        #     # transformer.eval()
        ...
        #     # print("making pipe")
        #     # self.pipeline = None
        #     # self.vae = vae.to(self.device_torch)
        #     # self.text_encoder = [text_encoder.to(self.device_torch), text_encoder_2.to(self.device_torch)]
        #     # self.tokenizer = [tokenizer, tokenizer_2]
        #     # self.unet = transformer.to(self.device_torch)
        ...
        #     flush()
            ...
            print("Loading Flux model")
            base_model_path = "/workspace/flux-dev"
            ...
            if os.path.exists(transformer_path):
                subfolder = None
                transformer_path = "/workspace/flux-dev/transformer"
                # check if the path is a full checkpoint.
                te_folder_path = os.path.join(model_path, 'text_encoder')
                # # if we have the te, this folder is a full checkpoint, use it as the base
                # if os.path.exists(te_folder_path):
        ...
        with network:
            with torch.no_grad():
                ...
                for i in tqdm(range(len(image_configs)), desc=f"Generating Images", leave=False):
                    ...
                    temp_prompt = gen_config.return_prompt(img, i)
                    val_tuple = (img, i, temp_prompt)
                    log_images.append(val_tuple)
                    
                if len(log_images) > 0:
                    gen_config = image_configs[0]
                    print('Exporting images to WanDB')
                    gen_config.log_all_images(log_images)
        ...
        if self.is_xl:
            ...
            if self.is_pixart:
                ...
                if self.is_flux:
                    ...
                    #     half_bs = bs // 2 if do_classifier_free_guidance else bs
                    ...
                    #     if do_classifier_free_guidance:
                    #         img = torch.cat([img[:half_bs], img[:half_bs]], dim=0)
                
                    #     img_ids = torch.zeros(h // 2, w // 2, 3)
                    #     img_ids[..., 1] += torch.arange(h // 2)[:, None]
                    #     img_ids[..., 2] += torch.arange(w // 2)[None, :]
                    #     img_ids = repeat(img_ids, "h w c -> b (h w) c", b=img.shape[0]).to(self.device_torch)
                
                    #     txt_ids = torch.zeros(img.shape[0], text_embeddings.text_embeds.shape[1], 3).to(self.device_torch)
                
                    #     txt = text_embeddings.text_embeds.to(self.device_torch)
                    #     vec = text_embeddings.pooled_embeds.to(self.device_torch)
                
                    #     if do_classifier_free_guidance:
                    #         txt_uncond, txt_cond = txt.chunk(2, dim=0)
                    #         vec_uncond, vec_cond = vec.chunk(2, dim=0)
                    #         txt = torch.cat([txt_uncond, txt_cond], dim=0)
                    #         vec = torch.cat([vec_uncond, vec_cond], dim=0)
                
                    #     t_vec = timestep.squeeze().float() / 1000
                    #     if t_vec.dim() == 0:
                    #         t_vec = t_vec.unsqueeze(0)
                    #     if t_vec.shape[0] == 1 and img.shape[0] > 1:
                    #         t_vec = t_vec.repeat(img.shape[0])
                
                    #     guidance_vec = torch.full((img.shape[0],), guidance_scale, device=t_vec.device, dtype=txt.dtype)
    ...
    def named_parameters(self, vae=True, text_encoder=True, unet=True, refiner=False, state_dict_keys=False) -> \
            OrderedDict[
                str, nn.Parameter]:
        named_params: OrderedDict[str, nn.Parameter] = OrderedDict()
        if vae:
